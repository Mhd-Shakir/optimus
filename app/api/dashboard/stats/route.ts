    ...
    try {
        ...
        events.forEach((event: any) => {
            ...
            if (Array.isArray(first)) {
                first.forEach((winner: any) => {
                    if (winner.studentId && winner.grade) {
                        processResult(winner.studentId, winner.grade, 'first');
                    }
                });
            } else if (first) {
                // Old format compatibility
                processResult(first, event.results.firstGrade, 'first');
            }

            // Process second place
            if (Array.isArray(second)) {
                second.forEach((winner: any) => {
                    if (winner.studentId && winner.grade) {
                        processResult(winner.studentId, winner.grade, 'second');
                    }
                });
            } else if (second) {
                // Old format compatibility
                processResult(second, event.results.secondGrade, 'second');
            }

            // Process third place
            if (Array.isArray(third)) {
                third.forEach((winner: any) => {
                    if (winner.studentId && winner.grade) {
                        processResult(winner.studentId, winner.grade, 'third');
                    }
                });
            } else if (third) {
                // Old format compatibility
                processResult(third, event.results.thirdGrade, 'third');
            }
