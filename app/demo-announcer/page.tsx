...
import { Loader2, Search, CheckCircle2, Trophy, ArrowRight, Mic2, LogOut } from "lucide-react";
import Link from "next/link";

export default function DemoAnnouncerDashboard() {
  ...
  const fetchData = async () => {
    try {
      ...
      // MOCK DATA
      const mockEvents = [
        { id: "e1", name: "Speech English", type: "Stage", category: "Protons", status: "completed" },
        { id: "e2", name: "Essay Writing", type: "Non-Stage", category: "Nexus", status: "announced" },
        { id: "e3", name: "Group Song", type: "Stage", category: "General-A", status: "completed" }
      ];
      const mockStats = { scores: { Ventus: 150, Ignis: 120 } };
      const mockHistory = [
        { count: 5, ventus: 40, ignis: 50 },
        { count: 10, ventus: 90, ignis: 85 }
      ];
      ...
      <nav className="fixed top-0 w-full z-50 bg-white/80 backdrop-blur-md border-b border-slate-200 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center gap-2 cursor-pointer" onClick={() => router.push('/demo-announcer')}>
              <div className="p-2 bg-emerald-100 rounded-lg">
                <Mic2 className="w-5 h-5 text-emerald-600" />
              </div>
              <span className="text-xl font-bold text-slate-900 tracking-tight">Optimus <span className="text-emerald-600 font-normal">Announcer</span> <span className="text-sm bg-amber-100 text-amber-700 px-2 py-0.5 rounded ml-2">DEMO</span></span>
            </div>
            
            <div className="flex items-center gap-4">
              <div className="hidden sm:flex flex-col items-end mr-4">
                <span className="text-sm font-bold text-slate-800">Demo Announcer</span>
