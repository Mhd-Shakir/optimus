...
export default function DemoTeamDashboard() {
  // MOCK AUTH
  const user = { team: "Team Ignis", role: "team" };
  ...
  const demoToast = (title = "Demo Mode 🛡️", desc = "Action simulated locally.") => {
  ...
  const fetchData = async () => {
    try {
      ...
      // MOCK DATA
      const mockEvents = [
        { _id: "e1", name: "Speech English", type: "Stage", category: "Protons", teamLimit: 3, groupEvent: false },
        { _id: "e2", name: "Essay Writing", type: "Non-Stage", category: "Nexus", teamLimit: 3, groupEvent: false },
        { _id: "e3", name: "Group Song", type: "Stage", category: "General-A", groupEvent: true }
      ];
      const mockStudents = [
        { _id: "s1", name: "Ahmed Raza", chestNo: "1024", team: "Team Ignis", category: "Protons", studentClass: "8", registeredEvents: [{ eventId: "e1", isStar: false }] },
        { _id: "s2", name: "John Doe", chestNo: "2048", team: "Team Ignis", category: "Nexus", studentClass: "10", registeredEvents: [{ eventId: "e2", isStar: true }] },
        { _id: "s3", name: "Sammy K", chestNo: "1150", team: "Team Ignis", category: "Protons", studentClass: "9", registeredEvents: [{ eventId: "e3", groupNo: 1, isStar: false }] }
      ];
  ...
  const confirmDelete = async () => {
    ...
    demoToast("Deleted", "Simulated deletion.");
  ...
  const handleRegister = async (e: any) => { 
    e.preventDefault(); 
    if (!formData.name) return toast({ variant: "destructive", title: "Missing Field", description: "Name is required." }); 
    if (!formData.studentClass) return toast({ variant: "destructive", title: "Missing Field", description: "Class is required." }); 
    setSubmitting(true); 
    setTimeout(() => {
        demoToast();
        closeModal();
  ...
  const handleGroupRegister = async (e: any) => {
    e.preventDefault();
    demoToast("Group Registered", "Simulated group registration.");
    cancelGroupEdit();
    setGroupParticipants([]);
    setGroupEventId("");
    setGroupNo(1);
  ...
  return (
    <div className="flex h-[100dvh] w-full bg-slate-50 font-sans overflow-hidden">
      <aside className="w-64 bg-white border-r border-slate-200 hidden md:flex flex-col h-full z-20 shrink-0">
        <div className="p-6 border-b border-slate-50"><h1 className="text-2xl font-black text-emerald-600">Optimus</h1><p className="text-[10px] text-slate-400 font-bold uppercase">Team Portal <Badge variant="secondary" className="ml-2 bg-yellow-200 text-yellow-800">DEMO</Badge></p></div>
